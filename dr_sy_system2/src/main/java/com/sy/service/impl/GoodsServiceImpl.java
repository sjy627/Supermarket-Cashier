...
public class GoodsServiceImpl implements GoodsService {
    ...
    @Override
    public ResultDto<Object> getLowStockGoods() {
        List<Goods> lowStockGoods = goodsMapper.selLowStockGoods();
        return new ResultDto<>(200,"获取库存预警商品成功",lowStockGoods);
    }

    @Override
    public ResultDto<Object> getReorderGoods() {
        List<Goods> reorderGoods = goodsMapper.selReorderGoods();
        return new ResultDto<>(200,"获取需要补货的商品成功",reorderGoods);
    }

    @Override
    public ResultDto<Object> updateGoodsStock(Goods goods) {
        if (goods == null || goods.getGNo() == null) {
            return new ResultDto<>(5001,"商品信息不能为空");
        }
        Integer rows = goodsMapper.updateGoodsStock(goods);
        if (rows > 0) {
            return new ResultDto<>(200,"更新库存成功");
        }
        return new ResultDto<>(5002,"更新库存失败");
    }

    @Override
    public ResultDto<Object> getStockWarningStats() {
        List<Goods> allGoods = goodsMapper.selAllGoods();
        List<Goods> lowStockGoods = goodsMapper.selLowStockGoods();
        List<Goods> reorderGoods = goodsMapper.selReorderGoods();

        Map<String, Object> stats = new HashMap<>();
        stats.put("totalGoods", allGoods.size());
        stats.put("lowStockCount", lowStockGoods.size());
        stats.put("reorderCount", reorderGoods.size());
        stats.put("normalCount", allGoods.size() - lowStockGoods.size());

        return new ResultDto<>(200,"获取库存统计成功",stats);
    }
